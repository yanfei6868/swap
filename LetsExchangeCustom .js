...
// 插件核心逻辑：使用 default export 导出完整的插件对象
export default {
    name: 'DemoSign',
    meta: {
        name: 'DemoSign',
    ...
    async init(adapter, env, utils) {
        utils.log('[init] 插件加载成功', { adapter: adapter.name });
        return { tsOffset: 0 }; // 缓存的状态，后续阶段能拿到
    },

    // 签名阶段（最常用）
    async sign(bodyStr, secret, prevSign, utils, state) {
    ...
    // 请求修补阶段（最常用）
    async flow(vars, chain, utils, state) {
        ...
        let bodyChanges = {};
        const pluginName = this.name; // 从导出的对象中获取 name
        ...
        if (chain.bodyStr && affiliateId != null) {
            ...
            try {
                ...
                // 记录错误，但不中断整个 flow 阶段
                utils.log?.(`[${pluginName}] Error parsing chain body for affiliate ID injection: ${e.message}`);
            ...
            utils.log?.(`[${pluginName}] affiliate_id injected into body: ${affiliateId}`);
        } else if (chain.bodyStr && affiliateId == null) {
            // 如果 body 存在但 affiliate_id 为空，则记录跳过信息
            utils.log?.(`[${pluginName}] Skip affiliate_id injection: vars.affiliate_id is null/undefined.`);
        }
    ...
    async parseResponse(data, vars, utils, state) {
